...
import { Users, UserCheck, CreditCard, MapPin, Phone, Banknote } from 'lucide-react';
import { Button } from './ui/Button';
import { Input } from './ui/Input';
import { Select } from './ui/Select';
import { Textarea } from './ui/Textarea';
import { Notification } from './ui/Notification';
import { fetchUsersForClient, createClient } from '../services/clientApi';
...
import { ClientFormData, ClientFormErrors, ClientNotificationState } from '../types/client';

export const ClientForm: React.FC = () => {
  const [users, setUsers] = useState<User[]>([]);
  const [loading, setLoading] = useState(false);
  const [loadingUsers, setLoadingUsers] = useState(true);
  const [notification, setNotification] = useState<ClientNotificationState>({
    show: false,
    type: 'success',
    message: ''
  });
  ...
  const loadUsers = async () => {
    try {
      ...
      const userData = await fetchUsersForClient();
      setUsers(userData);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showNotification('error', 'Failed to create client. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50 to-emerald-100 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Create a comprehensive client profile with all necessary details</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-green-600 to-emerald-600 px-8 py-6">
            ...
            <p className="text-green-100 mt-1">Fill in the details below to create a new client profile</p>
          </div>
